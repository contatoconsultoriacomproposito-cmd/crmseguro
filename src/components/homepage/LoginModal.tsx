...
export default function LoginModal({
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    try {
      ...
      /*
       * IMPORTANTE:
       * O AuthContext já validou:
       *
       * 1. autenticação no Supabase
       * 2. existência do perfil
       * 3. status ativo da conta
       *
       * Portanto, só chegamos aqui quando a conta
       * realmente está autorizada a entrar.
       */
      onClose?.()
      navigate("/dashboard")
    } catch (err: any) {
      setError(
        err?.message ||
          "Houve um erro ao acessar a conta."
      )
    } finally {
      setLoading(false)
    }
