  ...
  async function handleSalvarCampanha(e: React.FormEvent) {
    ...
    try {
      ...
      if (campanhaEmEdicao) {
        ...
        const { error } = await supabase
          .from('tab_campanhas_resumo') 
          .insert([{ ...dadosCampanhaBanco, total_enviados: 0 }]);
          ...
          {carregando ? (
            ...
            campanhas.map((campanha) => (
              ...
              >
                ...
                {/* Correção: e.stopPropagation() inserido para evitar disparar o onClick do container */}
                   ...
                   <div className="flex items-start gap-2.5 min-w-0 flex-1 mr-2">
                     <div className="mt-0.5 flex-shrink-0">
                       <input 
                         ...
                         disabled={!temEmail}
                         /* Correção: Sincronizado com a ação correta para evitar avisos de input controlado */
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 transition-opacity animate-fadeIn">
        <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl overflow-y-auto max-h-[90vh] transform scale-100 transition-transform">
          ...
          <form 
            onSubmit={(e) => {
              e.preventDefault();
              //console.log("Processando campanha para a corretora:", idCorretoraMae);
              handleSalvarCampanha(e);
            }} 
            className="space-y-4"
          >
            <div>
