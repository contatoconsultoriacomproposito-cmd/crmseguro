...
// Etapas padronizadas conforme seu código de sinistros
...
export default function ModalContato({ isOpen, onClose, cliente, onSuccess }: ModalContatoProps) {
  ...
  async function fetchDadosIniciais() {
    try {
      // 1. Busca Sinistros vinculados ao cliente (agora trazendo etapa_atual)
      ...
      // 2. Busca Histórico (Interações + Ocorrências)
  ...
  // Função para mudar o sinistro e já carregar a etapa atual dele
  ...
  async function salvarComercial() {
    ...
    try {
      ...
      finalizarSucesso();
    } catch (err: any) { setErro(err.message); } finally { setLoading(false); }
  }
  ...
  async function salvarSinistro() {
    ...
    // Validações de data obrigatória conforme sua regra
    if (etapaSinistro === 'Conclusão' && !dataRetornoSinistro) {
      return setErro("Para finalizar, informe a Data de Conclusão.");
    }
    if (['Cadastro', 'Avaliação', 'Solução'].includes(etapaSinistro) && !dataRetornoSinistro) {
        return setErro(`Informe uma data de retorno para a fase ${etapaSinistro}.`);
    }
    ...
    try {
      ...
      // 2. Atualiza a "Etapa Atual" e Status na tab_sinistros (O que você pediu)
  ...
  function finalizarSucesso() {
    ...
    setTimeout(() => {
      ...
      // Limpa formulários
      setTipoAcao(''); setTextoAcao(''); setRelatoSinistro('');
    }, 1500);
  }
            ...
            <div className="space-y-6">
              ...
              {abaAtiva === 'COMERCIAL' && (
                <div className="space-y-4 animate-in slide-in-from-left-4 duration-300">
                  ...
                  <select value={tipoAcao} onChange={(e) => setTipoAcao(e.target.value)} className="w-full bg-slate-100 dark:bg-zinc-800 border-none rounded-2xl p-4 text-sm font-bold outline-none ring-1 ring-transparent focus:ring-blue-500 transition-all">
                  ...
                  <textarea value={textoAcao} onChange={(e) => setTextoAcao(e.target.value)} placeholder="O que foi conversado com o cliente?" className="w-full bg-slate-100 dark:bg-zinc-800 border-none rounded-2xl p-4 text-sm min-h-[100px] outline-none focus:ring-1 focus:ring-blue-500" />
                  <div className="p-4 bg-blue-50/50 dark:bg-blue-900/10 rounded-2xl border border-blue-100/50">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <input type="date" value={dataRetorno} onChange={(e) => setDataRetorno(e.target.value)} className="bg-white dark:bg-zinc-800 border-none rounded-xl p-3 text-sm font-bold outline-none ring-1 ring-slate-100 focus:ring-blue-500" />
                      <input type="time" value={horarioRetorno} onChange={(e) => setHorarioRetorno(e.target.value)} className="bg-white dark:bg-zinc-800 border-none rounded-xl p-3 text-sm font-bold outline-none ring-1 ring-slate-100 focus:ring-blue-500" />
                    </div>
                  </div>
                  <button onClick={salvarComercial} disabled={loading} className="w-full bg-slate-900 dark:bg-blue-600 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:scale-[1.02] transition-all flex items-center justify-center gap-2 shadow-lg">{loading ? "Salvando..." : <><Save size={16} /> Registrar Comercial</>}</button>
                </div>
              )}
                        ...
                        <>
                          ...
                          <div className="p-4 bg-red-50/50 dark:bg-red-900/10 rounded-2xl border border-red-100/50">
                            <h4 className="text-[10px] font-black uppercase text-red-600 mb-3 flex items-center gap-2">
                              <Calendar size={14} /> {etapaSinistro === 'Conclusão' ? 'Data de Conclusão (Obrigatório)' : 'Data para Próximo Retorno'}
                            </h4>
                            <input type="date" value={dataRetornoSinistro} onChange={(e) => setDataRetornoSinistro(e.target.value)} className="w-full bg-white dark:bg-zinc-800 border-none rounded-xl p-3 text-sm font-bold outline-none ring-1 ring-red-50 focus:ring-red-500" />
                          </div>
