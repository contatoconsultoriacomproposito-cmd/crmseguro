...
interface EventoAgenda {
  ...
  extendedProps: {
    ...
    tipo: 'PF' | 'PJ';
    fase: string;
    origem: 'COMERCIAL' | 'SINISTRO' | 'RENOVACAO';
    itemId?: string;
  };
}
...
export default function AgendaCorretor() {
  ...
  const fetchCompromissos = useCallback(async () => {
    try {
      ...
      // --- BUSCA A: CLIENTES (Adicionado nome_fantasia na busca) ---
      ...
      resClientes.data?.forEach(cli => {
        // Prioriza Nome Fantasia -> Razão Social -> Nome genérico
      ...
      resRenovacoes.data?.forEach(renov => {
        ...
        // Identificação visual alinhada para PJ na Renovação
  ...
  // Apenas UM bloco de loading limpo e funcional
  if (loading) {
    return (
