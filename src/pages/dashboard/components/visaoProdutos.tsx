  ...
  const stats = useMemo(() => {
    ...
    propostasRaw.forEach(p => {
      ...
      p.tab_proposta_opcoes?.forEach((opcao: any) => {
        opcao.tab_proposta_itens?.forEach((item: any) => {
          ...
          // 2. FLUXO DE COTAÇÕES (Considera TODAS as cotações criadas no período)
          ...
          if (status === 'vendido') {
            ...
            // 4. PROJEÇÃO DE RENOVAÇÕES (Fechamento Futuro)
    ...
    return {
      ...
      graficoRenovacoes: Object.entries(acc.renovacoesFuturas)
        .sort()
        .filter(([mes]) => mes >= new Date().toISOString().substring(0, 7))
        .slice(0, 6)
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {stats.rankingProdutos.map((p: any) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* 2. PROJEÇÃO DE RENOVAÇÕES (CLARO) */}
        ...
        <div className="grid grid-rows-2 gap-6">
          ...
          <div className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm flex items-center">
            ...
            <div className="w-1/2 space-y-3 pl-6 border-l border-slate-100">
               {stats.graficoPagamentos.map((item) => (
                 <div key={item.name} className="flex justify-between items-center">
                    <span className="text-xs font-black text-slate-600 uppercase truncate pr-2">{item.name}</span>
                    <span className="text-sm font-black text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-lg">{item.value}</span>
                 </div>
               ))}
            </div>
          </div>

          {/* Fluxo de Cotações (Mês) - AJUSTADO */}
              ...
              {stats.graficoCotacoes.length > 0 ? stats.graficoCotacoes.slice(-8).map((item) => (
                <div key={item.name} className="flex-1 flex flex-col items-center gap-2 group">
                   <div 
                    ...
                    style={{ height: `${(item.value / Math.max(...stats.graficoCotacoes.map(g => g.value))) * 100}%` }}
                   >
                     <div className="absolute -top-6 left-1/2 -translate-x-1/2 text-[28px] font-black text-indigo-600 transition-opacity">
                        {item.value}
                     </div>
                   </div>
                ...
                <div className="w-full text-center text-sm font-bold text-slate-300 italic uppercase">Sem dados no período</div>
              )}
            </div>
          </div>
        </div>
