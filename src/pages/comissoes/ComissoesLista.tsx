...
import { Search, Calendar, ArrowDownCircle, CheckCircle, AlertTriangle, Check, Eye, Wallet, RefreshCcw, Landmark, ArrowUpRight, ArrowDownLeft, Equal, PlusCircle, Building2 } from 'lucide-react';
...
export const ComissoesLista = () => {
  ...
  // Filtro e Ordenação Alfabética Rigorosa por Nome do Cliente
  ...
  return (
    <div className="p-6 space-y-6 text-left bg-zinc-50/50 dark:bg-zinc-950 min-h-screen relative">
      ...
      <div className="bg-white dark:bg-zinc-900 p-4 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 flex flex-col lg:flex-row gap-4 items-center justify-between shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto justify-end">
          <div className="flex items-center gap-1 bg-zinc-50 dark:bg-zinc-950 px-3 py-1.5 rounded-2xl border border-zinc-200 dark:border-zinc-800 text-[11px] font-bold">
            ...
            <select value={tipoFiltroData} onChange={(e) => setTipoFiltroData(e.target.value as any)} className="bg-transparent outline-none uppercase font-black text-zinc-600 cursor-pointer mr-2">
              <option value="vencimento">Data Vencimento</option>
              <option value="venda">Data da Venda</option>
            </select>
            <input type="date" value={dataDe} onChange={(e) => setDataDe(e.target.value)} className="bg-transparent outline-none" />
            <span className="text-zinc-400 mx-1">ATÉ</span>
            <input type="date" value={dataAte} onChange={(e) => setDataAte(e.target.value)} className="bg-transparent outline-none" />
          </div>
                ...
                provisoesFiltradasEOrdenadas.map((p: ProvisaoItem) => (
                  <tr key={p.id} className={`${selecionadasLote.includes(p.id) ? 'bg-blue-50/50' : 'hover:bg-zinc-50/80'}`}>
                    ...
                    <td className="p-4 text-center font-black text-zinc-500 bg-zinc-50/40">
                      {p.numero_parcela} de {p.tab_comissoes_regras?.quantidade_parcelas || 1}
                    </td>
                    ...
                    <td className="p-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        {/* Botão de Visualização Reparado */}
                        <button 
      ...
      {/* MODAL 2: DETALHES E RAIO-X DA COMISSÃO (CORRIGIDO) */}
