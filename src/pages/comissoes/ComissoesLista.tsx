...
import { Search, Calendar, ArrowDownCircle, CheckCircle, AlertTriangle, Check, Eye, Wallet, RefreshCcw, Landmark, ArrowUpRight, ArrowDownLeft, Equal, PlusCircle, Building2, XCircle, Ban, ChevronDown, ChevronUp, Printer } from 'lucide-react';
...
export const ComissoesLista = () => {
  ...
  // Controle de linhas expandidas para o Raio-X abaixo do registro
  ...
  const alternarTodasAsLinhas = () => {
    if (linhasExpandidas.length === provisoesFiltradasEOrdenadas.length) {
      setLinhasExpandidas([]);
    } else {
      setLinhasExpandidas(provisoesFiltradasEOrdenadas.map(p => p.id));
    }
  };
  ...
  // Cria uma janela temporária em memória
  const janelaImpressao = window.open('', '_blank', 'width=1200,height=800');
  if (!janelaImpressao) {
    toast.error("Por favor, permita pop-ups para gerar o relatório de impressão.");
    return;
  }

  // Formata os blocos de dados que estão atualmente na tabela
  const linhasHtml = provisoesFiltradasEOrdenadas.map(p => {
    ...
    const clienteNome = p.tab_comissoes_regras?.tab_clientes?.razao_social || p.tab_comissoes_regras?.tab_clientes?.nome || '—';
      ...
      <tbody style="page-break-inside: avoid !important; break-inside: avoid !important;">
        <tr style="background-color: ${p.status_recebimento_seguradora === 'CANCELADO' ? '#f8fafc' : '#ffffff'}; color: ${p.status_recebimento_seguradora === 'CANCELADO' ? '#94a3b8' : '#334155'}; text-decoration: ${p.status_recebimento_seguradora === 'CANCELADO' ? 'line-through' : 'none'};">
          ...
          <td style="padding: 8px 10px; border-bottom: 1px solid #e2e8f0; text-align: center;">
            <span style="font-size: 7.5pt; font-weight: 800; padding: 2px 6px; border-radius: 3px; text-transform: uppercase; 
              background-color: ${statusLower === 'recebido' ? '#dcfce7' : statusLower === 'previsto' ? '#dbeafe' : '#fee2e2'}; 
              color: ${statusLower === 'recebido' ? '#15803d' : statusLower === 'previsto' ? '#1d4ed8' : '#b91c1c'};">
  ...
  // Montagem do documento de impressão completo
    ...
    <head>
      ...
      <style>
        @page {
          ...
          @bottom-right { content: "Página " counter(page) " de " counter(pages); font-family: Arial, sans-serif; font-size: 8pt; color: #71717a; }
          @bottom-left { content: "Relatório Financeiro Corporativo • Confidencial"; font-family: Arial, sans-serif; font-size: 8pt; font-weight: bold; color: #a1a1aa; }
        }
    ...
    <body>
      <div class="header">
        <div>
          ...
          <p>Módulo de Auditoria Integrada de Provisões</p>
        </div>
        <div class="meta">
          Data de Emissão: <strong>${new Date().toLocaleDateString('pt-BR')}</strong><br>
          Filtro Competência Vencimento: <strong>${vencimentoDe.split('-').reverse().join('/')} até ${vencimentoAte.split('-').reverse().join('/')}</strong>
        </div>
      </div>
      ...
      <div class="metrics">
        ...
        <div class="card"><div class="label" style="color: #047857;">Comissão Baixada</div><div class="val">${formatBRL(totalRecebidoCorretor)}</div></div>
        <div class="card"><div class="label" style="color: #b45309;">Saldo Restante</div><div class="val">${formatBRL(totalAReceberCorretor)}</div></div>
      </div>

      <div class="cash-flow">
        <div>Total Repasses (A): <strong>${formatBRL(ccPagoTotal)}</strong></div>
        <div>Total Liquidadas (B): <strong>${formatBRL(ccParcelasTotal)}</strong></div>
        <div>Saldo Atual em Caixa (A - B): <strong style="color: #059669;">${ccSaldoDiferenca >= 0 ? formatBRL(ccSaldoDiferenca) : `-${formatBRL(Math.abs(ccSaldoDiferenca))}`}</strong></div>
      </div>
      ...
      <script>
        // Dispara o comando de impressão assim que carregar o conteúdo
        window.onload = function() {
          window.print();
          // Fecha a aba extra logo após o término da ação
  ...
  return (
    <div className="p-6 space-y-6 text-left bg-zinc-50/50 dark:bg-zinc-950 min-h-screen relative print:p-0 print:bg-white">
      ...
      {/* BOTÕES SUPERIORES DE AÇÃO - ESCONDIDOS NA IMPRESSÃO */}
      <div className="flex justify-between items-center print:hidden">
        <div className="flex gap-2">
          ...
          >
            <Eye size={16} />
            {linhasExpandidas.length === provisoesFiltradasEOrdenadas.length ? 'Recolher Todos os Detalhes' : 'Expandir Todos os Detalhes (Modo Auditoria)'}
          </button>
        </div>

        <button 
          onClick={gerenciarImpressaoLayout}
          className="flex items-center gap-2 bg-emerald-600 text-white px-5 py-3 rounded-2xl text-xs font-black uppercase tracking-tight shadow-md hover:bg-emerald-500 transition-all"
        >
          <Printer size={16} />
          Imprimir / Gerar PDF
        </button>
      ...
      {/* BARRA FLUTUANTE DE LOTE - ESCONDIDA NA IMPRESSÃO */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 print:grid-cols-4 print:gap-2">
        <div className="bg-white dark:bg-zinc-900 p-5 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 flex items-center justify-between shadow-sm print:p-3 print:rounded-xl">
          <div>
            ...
            <h2 className="text-xl font-black text-zinc-950 dark:text-white mt-2 tracking-tight print:text-sm">{formatBRL(totalGeradoMae)}</h2>
          </div>
        ...
        <div className="bg-white dark:bg-zinc-900 p-5 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 flex items-center justify-between shadow-sm print:p-3 print:rounded-xl">
          <div>
            ...
            <h2 className="text-xl font-black text-zinc-950 dark:text-white mt-2 tracking-tight print:text-sm">{formatBRL(totalProvisionadoCorretor)}</h2>
          </div>
        ...
        <div className="bg-white dark:bg-zinc-900 p-5 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 flex items-center justify-between shadow-sm print:p-3 print:rounded-xl">
          <div>
            ...
            <h2 className="text-xl font-black text-emerald-600 mt-2 tracking-tight print:text-sm">{formatBRL(totalRecebidoCorretor)}</h2>
          </div>
        ...
        <div className="bg-white dark:bg-zinc-900 p-5 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 flex items-center justify-between shadow-sm print:p-3 print:rounded-xl">
          <div>
            ...
            <h2 className="text-xl font-black text-amber-600 mt-2 tracking-tight print:text-sm">{formatBRL(totalAReceberCorretor)}</h2>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 print:grid-cols-3 print:gap-2">
          <div className="bg-zinc-50 dark:bg-zinc-950 p-4 rounded-2xl flex items-center justify-between print:p-2 print:rounded-lg">
            <div>
              ...
              <span className="text-sm font-bold font-mono text-zinc-800 dark:text-zinc-200 print:text-xs">{formatBRL(ccPagoTotal)}</span>
            </div>
            <div className="p-2 bg-blue-50 text-blue-600 rounded-lg print:hidden"><ArrowDownLeft size={16} /></div>
          </div>
          <div className="bg-zinc-50 dark:bg-zinc-950 p-4 rounded-2xl flex items-center justify-between print:p-2 print:rounded-lg">
            <div>
              <span className="text-[10px] uppercase font-black text-zinc-400 block mb-1">Total de Parcelas Liquidadas (B)</span>
              <span className="text-sm font-bold font-mono text-zinc-800 dark:text-zinc-200 print:text-xs">{formatBRL(ccParcelasTotal)}</span>
            </div>
            <div className="p-2 bg-zinc-100 text-zinc-500 rounded-lg print:hidden"><ArrowUpRight size={16} /></div>
          </div>
          <div className={`p-4 rounded-2xl border flex items-center justify-between print:p-2 print:rounded-lg ${ccSaldoDiferenca < 0 ? 'bg-rose-50/50 border-rose-100 dark:bg-rose-950/20' : 'bg-emerald-50/50 border-emerald-100 dark:bg-emerald-950/20'}`}>
            <div>
              <span className="text-[10px] uppercase font-black text-zinc-400 block mb-1">Saldo Disponível em Caixa (A - B)</span>
              <span className={`text-base font-black font-mono ${ccSaldoDiferenca < 0 ? 'text-rose-600' : 'text-emerald-600'} print:text-xs`}>
                {ccSaldoDiferenca >= 0 ? formatBRL(ccSaldoDiferenca) : `-${formatBRL(Math.abs(ccSaldoDiferenca))}`}
              </span>
            </div>
            <div className={`p-2 rounded-lg ${ccSaldoDiferenca < 0 ? 'bg-rose-100 text-rose-600' : 'bg-emerald-100 text-emerald-600'} print:hidden`}><Equal size={16} /></div>
          </div>
        </div>
      </div>

      {/* BLOCO DE FILTROS EVOLUÍDO E MULTI-DATAS - ESCONDIDO NA IMPRESSÃO */}
      ...
      {/* GRADE DE LANÇAMENTOS COM AUDITORIA ACOPLADA */}
      <div className="bg-white dark:bg-zinc-900 rounded-[2rem] border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden print:border-none print:shadow-none">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs font-bold text-zinc-600 dark:text-zinc-400 print:text-[10px]">
            ...
            {loading ? (
              <tr>
                <td colSpan={11} className="p-8 text-center uppercase tracking-wider font-black text-zinc-400 animate-pulse">
                  Buscando lançamentos...
                </td>
              </tr>
            ) : provisoesFiltradasEOrdenadas.length === 0 ? (
              <tr>
                <td colSpan={11} className="p-8 text-center uppercase tracking-wider font-black text-zinc-400">
              ...
              provisoesFiltradasEOrdenadas.map((p: ProvisaoItem) => {
                ...
                return (
                  /* CORRIGIDO: Agora usando o Fragment importado diretamente */
                  <Fragment key={p.id}>
                    ...
                    <tr className={`${selecionadasLote.includes(p.id) ? 'bg-blue-50/50' : 'hover:bg-zinc-50/80'} ${p.status_recebimento_seguradora === 'CANCELADO' ? 'opacity-50 bg-zinc-100/40 dark:bg-zinc-950/10 line-through text-zinc-400' : ''} print:bg-transparent`}>
                      ...
                      <td className="p-4 font-black text-zinc-900 dark:text-white uppercase">
                        <div className="flex items-center gap-1.5">
                          <button 
                            onClick={() => toggleLinhaExpandida(p.id)} 
                      ...
                      <td className="p-4 text-center font-bold text-blue-600 bg-blue-50/30 print:bg-transparent">{p.data_recebimento ? p.data_recebimento.split('-').reverse().join('/') : '—'}</td>
                      ...
                      <td className="p-4 text-center print:hidden">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button 
                            ...
                            title="Visualizar Regra de Comissão Completa" 
                    ...
                    {/* SUB-LINHA ACOPLADA: RAIO-X COMPLETO PARA CONFERÊNCIA FÍSICA E IMPRESSÃO DE PDF */}
                    {estaExpandida && (
                        ...
                        <td colSpan={11} className="p-4 border-l-4 border-blue-500 print:col-span-9 print:p-2.5">
                          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-left text-zinc-700 dark:text-zinc-300 print:grid-cols-4 print:gap-2">
                            ...
                            <div className="text-[11px] space-y-1">
                              <span className="text-[9px] font-black uppercase text-zinc-400 tracking-wider block">Dados de Venda</span>
                              <p><span className="text-zinc-400">Data Venda:</span> <strong className="font-mono">{p.tab_comissoes_regras?.data_venda ? p.tab_comissoes_regras.data_venda.split('-').reverse().join('/') : '—'}</strong></p>
                              <p><span className="text-zinc-400">Contrato ID:</span> <span className="font-mono text-[10px]">{p.tab_comissoes_regras?.id ? `...${p.tab_comissoes_regras.id.slice(-8).toUpperCase()}` : '—'}</span></p>
                            </div>

                            <div className="text-[11px] space-y-1">
                              <span className="text-[9px] font-black uppercase text-zinc-400 tracking-wider block">Valores de Origem</span>
                              <p><span className="text-zinc-400">Base Cálculo:</span> <strong className="font-mono text-zinc-900 dark:text-white">{formatBRL(parseToNumber(p.tab_comissoes_regras?.base_calculo_valor))}</strong></p>
                              <p><span className="text-zinc-400">Comissão Total:</span> <span className="font-mono">{formatBRL(parseToNumber(p.valor_comissao_total))}</span></p>
                            </div>

                            <div className="text-[11px] space-y-1 bg-blue-50/40 dark:bg-blue-950/10 p-2 rounded-xl border border-blue-100/50 print:p-1 print:rounded-none print:border-none print:bg-transparent">
                              <span className="text-[9px] font-black uppercase text-blue-600 tracking-wider block">Split Corretor</span>
                              <p><span className="text-zinc-400">Percentual:</span> <strong className="font-mono text-blue-600">{parseToNumber(p.tab_comissoes_regras?.pct_corretor)}%</strong></p>
                              <p><span className="text-zinc-400">Líquido Corretor:</span> <strong className="font-mono text-blue-600">{formatBRL(parseToNumber(p.valor_direito_corretor))}</strong></p>
                            </div>

                            <div className="text-[11px] space-y-1">
                              <span className="text-[9px] font-black uppercase text-purple-600 tracking-wider block">Divisão Institucional</span>
                              <p><span className="text-zinc-400">Retido Mãe:</span> <span className="font-mono font-bold text-purple-600">{formatBRL(parseToNumber(p.valor_direito_corretora_mae))}</span></p>
                              {parseToNumber(p.valor_direito_parceiro) > 0 && (
                                <p><span className="text-zinc-400">Split Parceiro ({parseToNumber(p.tab_comissoes_regras?.pct_parceiro)}%):</span> <span className="font-mono font-bold">{formatBRL(parseToNumber(p.valor_direito_parceiro))}</span></p>
                              )}
                            </div>
      ...
      {/* MODAL 1: APORTE CAIXA - ESCONDIDO NA IMPRESSÃO */}
      {modalAporte && (
      ...
      {/* MODAL 2: RAIO-X DA COMISSÃO - MANTIDO APENAS COMO ALTERNATIVA INTERATIVA EM TELA */}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs z-50 flex items-center justify-center p-4 print:hidden">
          <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 rounded-[2rem] shadow-2xl max-w-lg w-full space-y-4 text-left">
            ...
            <div className="flex justify-between items-start">
              <div>
                <span className="text-[10px] font-black uppercase text-purple-600 bg-purple-50 dark:bg-purple-950/40 px-2.5 py-1 rounded-md">
                  Raio-X do Lançamento
                </span>
                <h3 className="text-xs font-black text-zinc-900 dark:text-white uppercase tracking-tight mt-2">
                  Origem da Regra de Comissão
                </h3>
              </div>
              <button 
                onClick={() => setModalDetalhe(false)} 
                className="text-zinc-400 hover:text-zinc-600 font-bold text-sm transition-colors focus:outline-none"
              >
                ✕
              </button>
            </div>
            ...
            <div className="p-4 bg-zinc-50 dark:bg-zinc-950 rounded-2xl space-y-2 text-xs border border-zinc-100 dark:border-zinc-800/50">
              <p>
                <span className="text-zinc-400 font-normal">Cliente:</span>{" "}
                <strong className="text-zinc-900 dark:text-white uppercase">
                  {obterNomeCliente(itemDetalhado.tab_comissoes_regras?.tab_clientes)}
                </strong>
              </p>
              <p>
                <span className="text-zinc-400 font-normal">Produto:</span>{" "}
                <span className="uppercase font-bold text-zinc-800 dark:text-zinc-200">
                  {itemDetalhado.tab_comissoes_regras?.base_produtos?.nome || "—"}
                </span>
              </p>
              <p>
                <span className="text-zinc-400 font-normal">Seguradora:</span>{" "}
                <span className="uppercase font-bold text-zinc-800 dark:text-zinc-200">
                  {itemDetalhado.tab_comissoes_regras?.base_seguradoras?.nome || "—"}
                </span>
              </p>
              <p>
                <span className="text-zinc-400 font-normal">Data da Venda:</span>{" "}
                <span className="font-mono font-bold text-zinc-800 dark:text-zinc-200">
                  {itemDetalhado.tab_comissoes_regras?.data_venda 
                    ? itemDetalhado.tab_comissoes_regras.data_venda.split("-").reverse().join("/") 
                    : "—"}
                </span>
              </p>
            </div>

            {/* Grid de Valores de Origem */}
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div className="p-3 border border-zinc-200 dark:border-zinc-800 rounded-xl bg-zinc-50/50 dark:bg-zinc-950/30">
                <span className="text-[9px] uppercase font-black text-zinc-400 block mb-1">Base de Cálculo</span>
                <span className="font-bold text-zinc-800 dark:text-zinc-200 font-mono">
                  {formatBRL(parseToNumber(itemDetalhado.tab_comissoes_regras?.base_calculo_valor))}
                </span>
              </div>
              <div className="p-3 border border-zinc-200 dark:border-zinc-800 rounded-xl bg-zinc-50/50 dark:bg-zinc-950/30">
                <span className="text-[9px] uppercase font-black text-zinc-400 block mb-1">Comissão Total Bruta</span>
                <span className="font-bold text-zinc-800 dark:text-zinc-200 font-mono">
                  {formatBRL(parseToNumber(itemDetalhado.valor_comissao_total))}
                </span>
              </div>
            </div>
            ...
            <div className="border-t border-zinc-100 dark:border-zinc-800 pt-3 space-y-2">
              <span className="text-[10px] font-black uppercase text-zinc-400 block mb-1">
                Divisão dos Splits (Acordo)
              </span>
              
              {/* Split Corretor */}
              <div className="flex justify-between items-center bg-blue-50/40 dark:bg-blue-950/20 p-2.5 rounded-xl border border-blue-100/50 dark:border-blue-900/30 text-xs">
                <span className="font-bold text-blue-700 dark:text-blue-400">
                  Direito do Corretor ({parseToNumber(itemDetalhado.tab_comissoes_regras?.pct_corretor)}%)
                </span>
                <span className="font-black text-blue-700 dark:text-blue-400 font-mono">
                  {formatBRL(parseToNumber(itemDetalhado.valor_direito_corretor))}
                </span>
              </div>
              
              {/* Retido Mãe */}
              <div className="flex justify-between items-center bg-purple-50/40 dark:bg-purple-950/20 p-2.5 rounded-xl border border-purple-100/50 dark:border-purple-900/30 text-xs">
                <span className="font-bold text-purple-700 dark:text-purple-400">
                  Retido pela Corretora Mãe
                </span>
                <span className="font-black text-purple-700 dark:text-purple-400 font-mono">
                  {formatBRL(parseToNumber(itemDetalhado.valor_direito_corretora_mae))}
                </span>
              </div>
              
              {/* Split Parceiro Opcional */}
              {parseToNumber(itemDetalhado.valor_direito_parceiro) > 0 && (
                <div className="flex justify-between items-center bg-zinc-100 dark:bg-zinc-800/60 p-2.5 rounded-xl text-xs border border-transparent dark:border-zinc-700/50">
                  <span className="font-bold text-zinc-600 dark:text-zinc-400">
                    Split Parceiro ({parseToNumber(itemDetalhado.tab_comissoes_regras?.pct_parceiro)}%)
                  </span>
                  <span className="font-black text-zinc-600 dark:text-zinc-300 font-mono">
                    {formatBRL(parseToNumber(itemDetalhado.valor_direito_parceiro))}
                  </span>
                </div>
              )}
            </div>

            {/* Botão de Cancelamento de Contrato Condicional */}
            {itemDetalhado.status_recebimento_seguradora !== "CANCELADO" && (
              <div className="pt-2 border-t border-zinc-100 dark:border-zinc-800">
                <button
                  onClick={() =>
                    cancelarProvisoesFuturasContrato(
                      itemDetalhado.tab_comissoes_regras?.id || "",
                      obterNomeCliente(itemDetalhado.tab_comissoes_regras?.tab_clientes)
                    )
                  }
                  className="w-full flex items-center justify-center gap-2 py-2.5 bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-900/40 font-black text-[10px] uppercase rounded-xl tracking-wider shadow-sm hover:bg-rose-100 dark:hover:bg-rose-950/50 transition-colors"
                >
            ...
            <button
              onClick={() => setModalDetalhe(false)}
              className="w-full py-2.5 bg-zinc-900 hover:bg-zinc-800 dark:bg-zinc-100 dark:hover:bg-white text-white dark:text-zinc-950 font-black text-[10px] uppercase rounded-xl tracking-wider shadow-sm transition-colors"
            >
              Fechar Diagnóstico
            </button>
